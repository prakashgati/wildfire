...
abstract class InMemoryBaseTable(
  ...
  case class InMemoryBatchScan(
    ...
    override def filter(filters: Array[Filter]): Unit = {
      filters.foreach { f =>
        ...
        val partitionColNames = partitioning.flatMap(t =>
          t.references().map(ne => ne.fieldNames().reduce(_ + "." + _))
        )

        if (partitioning.length == 1 && partitionColNames.exists(_ == colName)) {
          val ref = partitioning.head.references().head
          f match {
            case In(attrName, valueArray) if valueArray.length == 1 && valueArray(0) != null &&
              valueArray(0).getClass.getName.indexOf("BroadcastedJoinKeysWrapperImpl") != -1 =>
            ...
            case In(attrName, values) if attrName == ref.toString =>
              val matchingKeys = values.map { value => if (value != null) value.toString else null
              }.toSet
              this.data = this.data.filter(partition => {
        ...
        this.broadcastVarFilters = this.broadcastVarFilters ++ filters.filter {
          case In(_, values) if values.length == 1 && values(0) != null &&
            values(0).getClass.getName.indexOf("BroadcastedJoinKeysWrapperImpl") != -1 => true
    ...
    override def getPushedBroadcastFilters(): java.util.List[PushedBroadcastFilterData] = {
      ...
      new util.ArrayList[PushedBroadcastFilterData](
        this.broadcastVarFilters.map(f => {
          ...
          // scalastyle:off classforname
          val constr = clazz.getConstructor(classOf[String],
            Class.forName(BROADCASTED_JOIN_KEYS_WRAPPER_CLASS))
    ...
    override def allAttributes(): Array[NamedReference] = readSchema.fields.map(
      sf => FieldReference(sf.name))

     override def getPushedBroadcastVarIds(): java.util.Set[java.lang.Long] = {
      import scala.collection.JavaConverters._
      this.getPushedBroadcastFilters().asScala.map(x =>
        java.lang.Long.valueOf(x.bcVar.getBroadcastVarId)).toSet.asJava
