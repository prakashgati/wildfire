              ...
              val finalPartitions = outputPartitioning match {
                case p: KeyGroupedPartitioning =>
                  if (conf.v2BucketingPushPartValuesEnabled &&
                    ...
                    val groupedPartitions = groupPartitions(filteredPartitions.map(_.head),
                      groupSplits = true).getOrElse(Seq.empty)

                    // This means the input partitions are not grouped by partition values.
                    // We'll need to check `groupByPartitionValues` and decide whether to group and
                    // replicate splits within a partition.
                    if (spjParams.commonPartitionValues.isDefined &&
                      ...
                      val nestGroupedPartitions = groupedPartitions.map {
                        case (partValue, splits) =>
                          ...
                          assert(numSplits.isDefined, s"Partition value $partValue does not exist" +
                            s" in common partition values from Spark plan")

                          val newSplits = if (spjParams.replicatePartitions) {
                            // We need to also replicate partitions according to the other side
                            // of join
                            Seq.fill(numSplits.get)(splits)
                          } else {
                            // Not grouping by partition values: this could be the side with
                            // we'll need to check if the final number of splits of a partition is
                            // smaller than the original number, and fill with empty splits if so.
                            // This is necessary so that both partially clustered distribution.
                            // Because of dynamic filtering, sides of a join will have the same
                            // number of partitions & splits.
                      ...
                      // In case `commonPartitionValues` is not defined (e.g., SPJ is not used),
                      // there could exist duplicated partition values, as partition grouping is
                      // not done at the beginning and postponed to this method. It is important
                      // to use unique partition values here so that grouped partitions won't
                      // get duplicated.
                    ...
                    val partitionMapping = filteredPartitions.map { parts =>
                      val row = parts.head.asInstanceOf[HasPartitionKey].partitionKey()
                      InternalRowComparableWrapper(row, p.expressions) -> parts
                    }.toMap
                    p.partitionValues.map { partValue =>
                      // Use empty partition for those partition values that are not present
                      partitionMapping.getOrElse(
                        InternalRowComparableWrapper(partValue, p.expressions), Seq.empty)
                    }
...
object BatchScanExec {
  val PRESERVE_BATCH_EXEC_TO_USE: TreeNodeTag[BatchScanExec] =
    TreeNodeTag("preserve_batch_scan_exec")
}
